...
func process(conn net.Conn){
	...
	//输入自己的身份
	UserInput,_:=netRead(conn)
	CurrUsername, ok :=userDB [UserInput]
	if ok{
		inputInfo := "您已成功连接 :" + CurrUsername+"\n\r"
		...
		if err != nil {
			...
			//输入对方的身份
			UserInputOtherCode,_:=netRead(conn)
			OtherUsername, _ :=userDB [UserInputOtherCode]

			go writeMessage(conn,user{code: UserInputOtherCode,username: OtherUsername},rabbitmq_conn,addr)
			go acceptMessage(conn,user{code: UserInput,username: CurrUsername},rabbitmq_conn,addr)
		}
	}else {
		inputInfo := "您已成功连接，陌生人\n\r"
		_, err := conn.Write([]byte(inputInfo)) // 发送数据
		if err != nil {
			return
		}
...
func netRead(conn net.Conn) (string,error) {
	...
	recvStr:= strings.Trim(string(buf[:n]), "\r\n")
